...
import org.fosstrak.ale.wsdl.alecc.epcglobal.ArrayOfString;
import org.fosstrak.ale.wsdl.alecc.epcglobal.Define;
import org.fosstrak.ale.wsdl.alecc.epcglobal.DuplicateNameExceptionResponse;
import org.fosstrak.ale.wsdl.alecc.epcglobal.DuplicateSubscriptionExceptionResponse;
import org.fosstrak.ale.wsdl.alecc.epcglobal.CCSpecValidationExceptionResponse;
import org.fosstrak.ale.wsdl.alecc.epcglobal.EmptyParms;
import org.fosstrak.ale.wsdl.alecc.epcglobal.GetCCSpec;
import org.fosstrak.ale.wsdl.alecc.epcglobal.GetSubscribers;
import org.fosstrak.ale.wsdl.alecc.epcglobal.Immediate;
import org.fosstrak.ale.wsdl.alecc.epcglobal.ImplementationExceptionResponse;
import org.fosstrak.ale.wsdl.alecc.epcglobal.InvalidURIExceptionResponse;
import org.fosstrak.ale.wsdl.alecc.epcglobal.NoSuchNameExceptionResponse;
import org.fosstrak.ale.wsdl.alecc.epcglobal.NoSuchSubscriberExceptionResponse;
import org.fosstrak.ale.wsdl.alecc.epcglobal.Poll;
import org.fosstrak.ale.wsdl.alecc.epcglobal.SecurityExceptionResponse;
import org.fosstrak.ale.wsdl.alecc.epcglobal.Subscribe;
import org.fosstrak.ale.wsdl.alecc.epcglobal.Undefine;
import org.fosstrak.ale.wsdl.alecc.epcglobal.Unsubscribe;
 ...
 * @author swieland
 */
public class ALECCClient extends AbstractTab {
	 ...
	 */
	public static final String CFG_ENDPOINT = "org.fosstrak.ale.client.alecc.endpoint";
	 ...
	 */
	private JCheckBox m_createCommandSink;
	...
	// logger.
	private static final Logger s_log = Logger.getLogger(ALECCClient.class);
	
	private static final int CMD__DEFINE = 1;
	private static final int CMD__UNDEFINE = 2;
	private static final int CMD__GET_CCSPEC = 3;
	private static final int CMD__GET_CCSPEC_NAMES = 4;
	 ...
	 */
	public ALECCClient(JFrame parent) throws SecurityException, NoSuchMethodException {
		super(ALECCServicePortType.class, CFG_ENDPOINT, parent, ALECCServicePortType.class.getMethod("getStandardVersion", EmptyParms.class), new EmptyParms());
	}

	@Override
	public String getBaseNameKey() {
		return "org.fosstrak.ale.client.alecc.lang.base";
	}
	...
	protected void setCommandPanel(int command) {
		...
		case CMD__GET_CCSPEC_NAMES: // getECSpecNames
		...
		case CMD__SUBSCRIBE: // subscribe
			...
			m_createCommandSink = new JCheckBox();
			JLabel lbl = new JLabel(m_guiText.getString("CreateCommandSink"));
			lbl.setFont(m_font);
			m_commandPanel.add(lbl);
			m_commandPanel.add(m_createCommandSink);
	 ...
	 */
	private void addCCSpecNameComboBox(JPanel panel) {
		...
		try {
			ccSpecNames = getAleServiceProxy().getCCSpecNames(new EmptyParms()).getString();
		} catch (Exception e) {
		}
		if (ccSpecNames != null && ccSpecNames.size() > 0) {
	...
	protected void executeCommand() {
		...
		try {
			...
			case CMD__GET_CCSPEC_NAMES: // getECSpecNames
				result = getAleServiceProxy().getCCSpecNames(new EmptyParms());
				break;
			...
			case CMD__GET_SUBSCRIBERS: // getSubscribers
				...
				case CMD__GET_CCSPEC: // getECSpec
					GetCCSpec getCCSpecParms = new GetCCSpec();
					getCCSpecParms.setSpecName(specName);
					result = getAleServiceProxy().getCCSpec(getCCSpecParms);
					break;
			...
			case CMD__UNSUBSCRIBE: // unsubscribe
				...
				case CMD__SUBSCRIBE:
					...
					if (m_createCommandSink.isSelected()) {
			...
			case CMD__IMMEDIATE: // immediate
				...
				CCSpec ccSpec;
				try {
					ccSpec = getCCSpecFromFile(filePath);
			...
			} else if (e instanceof CCSpecValidationExceptionResponse) {
		...
		try {
			ecSpecNames = getAleServiceProxy().getCCSpecNames(new EmptyParms()).getString();
		} catch (Exception e) {
		}
	 ...
	 */
	private CCSpec getCCSpecFromFile(String filename) throws Exception {
		FileInputStream inputStream = new FileInputStream(filename);
		return DeserializerUtil.deserializeCCSpec(inputStream);
	...
	protected void decodeResult(StringBuffer sb, Object result) {
		...
		} else if (result instanceof CCSpec) {
			CharArrayWriter writer = new CharArrayWriter();
			try {
				SerializerUtil.serializeCCSpec((CCSpec) result, writer);
			} catch (Exception e) {
				FosstrakAleClient.instance().showExceptionDialog(m_guiText.getString("SerializationExceptionMessage"));
			}
			sb.append(writer.toString());

		} else if (result instanceof CCReports) {
			CharArrayWriter writer = new CharArrayWriter();
			try {
				SerializerUtil.serializeCCReports((CCReports) result, writer);
			} catch (Exception e) {
				FosstrakAleClient.instance().showExceptionDialog(m_guiText.getString("SerializationExceptionMessage"));
			}
			sb.append(writer.toString());
